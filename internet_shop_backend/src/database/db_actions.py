...
from src.database.models import Product, Reveiw, User
...
def add_product(name: str, descripnion: str, price: float, img_url:str):
    product = Product(
        id=uuid4().hex,
        name=name,
    ...
    db.session.commit()
    return f"Товар '{name}' було успішно додано!"
...
def update_product(product_id: str, name: str, description: str, price: float, img_url: str) -> str:
    ...
    db.session.commit()
    return f"Товар з id '{product_id}' було успішно оновлено!"
...
def add_review_product(product_id: str, text: str, name: str) -> str:
    ...
    db.session.commit()
    return "Ви успішно залишили свій відгук!"
